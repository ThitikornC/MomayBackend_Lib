...
const PM_airLib = mongoose.model('pm_airlib', px_pm3250_schema);
const PM_iralib = mongoose.model('pm_iralib', px_pm3250_schema);
...
// ================= ESP Receivers =================
// รับข้อมูลจาก ESP ชื่อ PM_airLib
app.post('/esp/PM_airLib', async (req, res) => {
  try {
    const payload = req.body || {};
    const doc = await saveESPDoc(PM_airLib, payload);
    console.log('💾 PM_airLib saved:', doc._id);
    res.status(201).json({ success: true, id: doc._id });
  } catch (err) {
    console.error('❌ /esp/PM_airLib error:', err);
    res.status(500).json({ error: err.message });
  }
});

// Allow PUT as an alternative to POST for devices that use HTTP PUT
app.put('/esp/PM_airLib', async (req, res) => {
  try {
    const payload = req.body || {};
    const doc = await saveESPDoc(PM_airLib, payload);
    console.log('💾 PM_airLib saved (PUT):', doc._id);
    res.status(201).json({ success: true, id: doc._id });
  } catch (err) {
    console.error('❌ PUT /esp/PM_airLib error:', err);
    res.status(500).json({ error: err.message });
  }
});

// รับข้อมูลจาก ESP ชื่อ PM_iralib
app.post('/esp/PM_iralib', async (req, res) => {
  try {
    const payload = req.body || {};
    const doc = await saveESPDoc(PM_iralib, payload);
    console.log('💾 PM_iralib saved:', doc._id);
    res.status(201).json({ success: true, id: doc._id });
  } catch (err) {
    console.error('❌ /esp/PM_iralib error:', err);
    res.status(500).json({ error: err.message });
  }
});

// Allow PUT as an alternative to POST for devices that use HTTP PUT
app.put('/esp/PM_iralib', async (req, res) => {
  try {
    const payload = req.body || {};
    const doc = await saveESPDoc(PM_iralib, payload);
    console.log('💾 PM_iralib saved (PUT):', doc._id);
    res.status(201).json({ success: true, id: doc._id });
  } catch (err) {
    console.error('❌ PUT /esp/PM_iralib error:', err);
    res.status(500).json({ error: err.message });
  }
});

// Recent fetch endpoints for quick testing
app.get('/esp/PM_airLib/recent', async (req, res) => {
  try {
    const limit = parseInt(req.query.limit) || 20;
    const docs = await PM_airLib.find().sort({ timestamp: -1 }).limit(limit).lean();
    ...
    console.error('❌ GET /esp/PM_airLib/recent error:', err);
    res.status(500).json({ error: err.message });
  }
});

app.get('/esp/PM_iralib/recent', async (req, res) => {
  try {
    const limit = parseInt(req.query.limit) || 20;
    const docs = await PM_iralib.find().sort({ timestamp: -1 }).limit(limit).lean();
    const data = docs.map(d => ({
      timestamp: d.timestamp ? new Date(d.timestamp).toISOString() : null,
      mac_address: d.mac_address || null,
      voltage: d.voltage || null,
    ...
    console.error('❌ GET /esp/PM_iralib/recent error:', err);
    res.status(500).json({ error: err.message });
  }
});

// GET /daily-energy
// Returns active_power_total and timestamp from both pm_airlib and pm_iralib for a given date
app.get('/daily-energy', async (req, res) => {
  try {
    ...
    const [airDocsRaw, iraDocsRaw] = await Promise.all([
      PM_airLib.find({ timestamp: { $gte: start, $lte: end } })
        .sort({ timestamp: 1 })
        .limit(limit),
      PM_iralib.find({ timestamp: { $gte: start, $lte: end } })
        .sort({ timestamp: 1 })
        .limit(limit)
    ]);
    ...
    res.json({
      date: queryDate,
      series: [
        { label: 'PM_airLib', points: airSeries },
        { label: 'PM_iralib', points: iraSeries }
      ]
    });
  } catch (err) {
...
app.get('/daily-energy/:source', async (req, res) => {
  try {
    const source = (req.params.source || '').toLowerCase(); // e.g. pm_airlib or pm_iralib
    ...
    let Model;
    if (source === 'pm_airlib' || source === 'pm_air_lib' || source === 'pm-airlib') Model = PM_airLib;
    else if (source === 'pm_iralib' || source === 'pm_ira_lib' || source === 'pm-iralib') Model = PM_iralib;
    else return res.status(400).json({ error: 'Unknown source. Use pm_airlib or pm_iralib' });
...
app.patch('/esp/PM_airLib/:id', async (req, res) => {
  try {
    ...
    const doc = await PM_airLib.findByIdAndUpdate(id, updates, { new: true });
    if (!doc) return res.status(404).json({ error: 'Document not found' });
    res.json({ success: true, doc });
  } catch (err) {
    console.error('❌ PATCH /esp/PM_airLib/:id error:', err);
    res.status(500).json({ error: err.message });
  }
});

app.patch('/esp/PM_iralib/:id', async (req, res) => {
  try {
    const id = req.params.id;
    const allowed = [
      'voltage','current','power','active_power_phase_a','active_power_phase_b','active_power_phase_c',
    ...
    if (!Object.keys(updates).length) return res.status(400).json({ error: 'No allowed fields to update' });
    if (updates.timestamp) updates.timestamp = new Date(updates.timestamp);
    const doc = await PM_iralib.findByIdAndUpdate(id, updates, { new: true });
    if (!doc) return res.status(404).json({ error: 'Document not found' });
    res.json({ success: true, doc });
  } catch (err) {
    console.error('❌ PATCH /esp/PM_iralib/:id error:', err);
    res.status(500).json({ error: err.message });
  }
});
...
app.get('/daily-bill', async (req, res) => {
    try {
        ...
        // Use PM_airLib and prefer active_power_total when available
        const data = await PM_airLib.find({ timestamp: { $gte: start, $lte: end } })
              .sort({ timestamp: 1 })
...
app.get('/daily-diff', async (req, res) => {
    try {
        ...
        const getDailyEnergy = async (dateStr) => {
          ...
          const dayData = await PM_airLib.find({ timestamp: { $gte: start, $lte: end } })
                           .sort({ timestamp: 1 })
                          .select('active_power_total timestamp');
...
app.get('/minute-power-range', async (req, res) => {
    try {
        ...
        const data = await PM_airLib.find({
...
async function checkDailyPeak() {
  try {
    const latest = await PM_airLib.findOne().sort({ timestamp: -1 }).select('active_power_total timestamp');
    if (!latest) return;
...
app.get('/api/notifications/all', async (req, res) => {
  try {
    ...
    const peakNoti = await PeakNotificationH.find(query)
      .sort({ timestamp: -1 })
      .limit(parseInt(limit))
      .skip(skip)
      .lean();
    
    const dailyDiffNoti = await DailyDiffNotificationH.find(query)
      .sort({ timestamp: -1 })
      .limit(parseInt(limit))
      .skip(skip)
      .lean();
    
    const dailyBillNoti = await DailyBillNotificationH.find(query)
      .sort({ timestamp: -1 })
      .limit(parseInt(limit))
      .skip(skip)
      .lean();
    
    const testNoti = await TestNotificationH.find(query)
      .sort({ timestamp: -1 })
      .limit(parseInt(limit))
      .skip(skip)
      .lean();
    ...
    const totalPeak = await PeakNotificationH.countDocuments(query);
    const totalDailyDiff = await DailyDiffNotificationH.countDocuments(query);
    const totalDailyBill = await DailyBillNotificationH.countDocuments(query);
...
// source can be: 'px_pm3250', 'pm_airlib', 'pm_iralib'
app.get('/daily-energy/:source', async (req, res) => {
  try {
    ...
    let Model;
    if (source === 'px_pm3250') Model = PM_airLib;
    else if (source === 'pm_airlib') Model = PM_airLib;
    else if (source === 'pm_iralib') Model = PM_iralib;
    else return res.status(400).json({ error: 'Unknown source. Use px_pm3250, pm_airlib or pm_iralib' });
...
app.get('/esp/:source', async (req, res) => {
  try {
    ...
    // map URL source to model
    const s = source.toLowerCase();
    let Model = null;
    if (s === 'pm_airlib' || s === 'pm-airlib' || s === 'pm_air_lib' || source === 'PM_airLib') Model = PM_airLib;
    else if (s === 'pm_iralib' || s === 'pm-iralib' || s === 'pm_ira_lib' || source === 'PM_iralib') Model = PM_iralib;
    else return res.status(404).json({ error: 'Unknown source. Use PM_airLib or PM_iralib' });
